...
public class MainActivity extends MenuActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        testLeague.leagueNumber = 0;
        testLeague.leagueName = "";
        myRef.child("Leagues").child("League " + testLeague.leagueNumber).setValue(testLeague);
        ...
        if(userName_et.getText().toString().trim().length()==0 || password_et.getText().toString().trim().length()==0)
            ...
            q.addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    ...
                    if (!userTaken)
                    {
                        Subscriber newSub = new Subscriber(userName_et.getText().toString(),
                                                            password_et.getText().toString());
                        myRef.child("App Users").child(userName_et.getText().toString()).setValue(newSub);
                        Toast.makeText(MainActivity.this, "Registered successfully",Toast.LENGTH_LONG).show();
                        nowConnected = newSub;
                        userName = nowConnected.getSubName();
                        Random rnd = new Random();
                        myRef.child("test").setValue(rnd.nextInt());
                        Intent i1 = new Intent(MainActivity.this, RegisterRecords.class);
                        Bundle b = new Bundle();
                        b.putSerializable("currentSub", nowConnected.getSubName());
                        i1.putExtras(b);
                        startActivity(i1);
                    }
                }
